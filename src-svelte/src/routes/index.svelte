...
<!-- Options Modal -->
<div id="options-modal" class="m-10 h-full w-full z-50 absolute font-mono w-4/6 {options.display ? "visible" : "hidden"} flex-row rounded-lg px-10 py-3 bg-black bg-opacity-90 text-white">
    ...
    <div id="menu" class="bg-slate-800 text-white rounded-lg flex flex-col absolute justify-center text-center left-10 top-52 p-10 py-24 gap-8 bg-opacity-75">
        ...
        {#each ['Start', "Continue", 'Options', 'Exit'] as option}
            {#if titleScreen.disabledMenuOptions.includes(option)}
                <a class="text-gray-500">{option}</a>
            {:else}
                <a on:click|preventDefault={_ => menuSelect(option)}>{option}</a>
            {/if}
        {/each}
